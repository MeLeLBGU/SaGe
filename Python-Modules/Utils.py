...
def calculate_token_offset(i, last_times_accumulate_offset, tokens_in_line, updated_tokens_in_line, log2):
    ...
    if current_index == len(updated_tokens_in_line):
        ...
        if (current_index >= len(updated_tokens_in_line)):
            log2.info("Fail! current index = {}, length of \"updated_tokens_in_line\" = {}".format(current_index, len(updated_tokens_in_line)))
            log2.info("token #{}, updated tokens:{}".format(i, updated_tokens_in_line))
            current_index -= 1
        else:
...
def sg_for_window_mp(target_token, window, target_embeddings, context_embeddings, log2):
    current_p = 0
    for w in window:
        ...
        try:
            ...
            pass
            #log2.info("sigmoid error on token {}, when current_p={}".format(target_token, current_p))

    return (-1) * current_p


def substract_windows_from_sg_mp(i, tokens_in_line_ints, current_sg, target_embeddings, context_embeddings, window_size, log2):
    ...
    for index in range(context_start, context_end):
        ...
        sg_wo -= sg_for_window_mp(tokens_in_line_ints[index], window, target_embeddings, context_embeddings, log2)
...
def add_windows_to_sg_mp(model, updated_i, offset, updated_context_start, \
                        ...
                        target_embeddings, context_embeddings, window_size, log2):
    sg_wo = current_sg
    ...
    for index in range(updated_context_start, updated_i):
        try:
            ...
            sg_wo += sg_for_window_mp(updated_tokens_in_line[index], window, target_embeddings, context_embeddings, log2)
        except:
...
def update_sg_per_instance_of_token_mp(model, token_to_ablate, i, \
                                    ...
                                    target_embeddings, context_embeddings, log2, window_size):
    ...
    sg_wo = substract_windows_from_sg_mp(i, tokens_in_line_ints, sg_wo, target_embeddings, context_embeddings, window_size, log2)
    ...
    # calculating 'offset' = number of tokens added because of our new encoding
    offset = calculate_token_offset(i, last_times_accumulate_offset, tokens_in_line_pieces, updated_tokens_in_line, log2)
    ...
    if updated_window[:first_part_of_window_length] != original_window[:first_part_of_window_length]:
        # if we have former instance of the token in the window of current instance, ignore this computation
        #if not token_to_ablate in original_window:
            #''' add this (first log line) back !!!! '''
            #log2.info("---- ignored: not equal - token {}, original index {}, updated {} ----".format(token_to_ablate, i, updated_i))
            #self._log2.info("updated_window  {}+{}, (offset {}, last_times {})".format(updated_tokens_in_line[updated_context_start:updated_i], updated_tokens_in_line[updated_i+offset+1:updated_context_end], offset, last_times_accumulate_offset))
            #self._log2.info("original_window {}".format(original_window))
            #self._log2.info("updated tokens - {}".format(updated_tokens_in_line))
            #self._log2.info("original tokens- {}".format(tokens_in_line))
        # else : this is because some encoding of tokens before our token to ablate changed. we ignore this case too
    ...
    sg_wo = add_windows_to_sg_mp(model, updated_i, offset, updated_context_start, \
                                ...
                                sg_wo, target_embeddings, context_embeddings, window_size, log2)
...
def get_diff_sg_wo_token_for_line(model, line_index, \
                                    ...
                                    token_to_ablate, current_vocab, target_embeddings, context_embeddings, log2, window_size):
    sg_wo_diff = 0
    ...
    for i in indices:
        ...
        sg_wo_diff, last_time_offset = update_sg_per_instance_of_token_mp(model, token_to_ablate, i, line, tokens_in_line_ints, tokens_in_line_pieces, last_times_accumulate_offset, sg_wo_diff, current_vocab, target_embeddings, context_embeddings, log2, window_size)
        last_times_accumulate_offset += last_time_offset

    return token_to_ablate, sg_wo_diff

def sg_wo_token_mp(model, token_to_ablate, current_total_sg, current_vocab, training_filepath, target_embeddings, context_embeddings, log2, corpus_lines, window_size):
    sg_wo = current_total_sg
    ...
    for line in corpus_lines:
        ...
        for i in indices:
            ...
            sg_wo, last_time_offset = update_sg_per_instance_of_token_mp(model, token_to_ablate, i, line, tokens_in_line_ints, tokens_in_line_pieces, last_times_accumulate_offset, sg_wo, current_vocab, target_embeddings, context_embeddings, log2, window_size)
            last_times_accumulate_offset += last_time_offset
