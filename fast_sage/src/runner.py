...
from src.model import SaGeTokenizer


def run_tokenizer(experiment_name, corpus_filepath, partial_corpus_filepath, partial_corpus_line_number,
                  vocabulary_filepath, max_len, workers_number, full_vocab_schedule, embeddings_schedule,
                  random_seed, word2vec_params):
    utils.init_logger(experiment_name)
    logging.info(f"Start experiment {experiment_name}")
    logging.info(f"Process will use up to {workers_number} worker threads.")

    logging.info("Getting output directories")
    embeddings_folder, stats_folder, vocab_folder = utils.get_output_folder(experiment_name)
    logging.info("Setting random seed")
    utils.set_random_seed(experiment_name, random_seed)
    logging.info(f"Loading initial vocabulary from file {vocabulary_filepath}")
    byte_vocab = utils.load_vocab(vocabulary_filepath)
    ...
    if max_len != actual_max_len:
        logging.warning(f"max_len parameter value {max_len} doesn't match actual max {actual_max_len}")

    logging.info("Initializing tokenizer")
    sage_model = SaGeTokenizer(byte_vocab, max_len)

    logging.info(f"Loading Corpus from {corpus_filepath}")
    partial_corpus = utils.load_corpus(corpus_filepath, partial_corpus_filepath, partial_corpus_line_number)
    logging.info("Starting the training loop")
    vocab_schedule = full_vocab_schedule
    ...
    embedding_sizes = set(embeddings_schedule)

    # initialize embeddings for first iteration
    embeddings = emb.get_embeddings(vocab_schedule[0], embeddings_folder, partial_corpus, sage_model, workers_number, word2vec_params)
    ...
    while i < len(vocab_schedule) - 1:
        ...
        if vocab_schedule[i] in embedding_sizes:
            embeddings = emb.get_embeddings(current_step_vocab_size, embeddings_folder, partial_corpus, sage_model, workers_number, word2vec_params)
        ...
        total_tokens, total_triples, token_to_losses, ablated_sizes = utils.run_sage_parallel(embeddings, partial_corpus, sage_model, workers_number)
        ...
        utils.save_sorted_losses(sage_model, sorted_losses, target_vocab_size, vocab_folder)
        ...
        utils.save_stats(stats, stats_folder, target_vocab_size)
